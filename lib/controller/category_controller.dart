...
class CategoryController extends GetxController implements GetxService {
  ...
  void getSubCategoryList2(String restId) async {
    _subCategoryIndex = 0;
    _subCategoryList = null;
    _categoryProductList = null;
    Response response = await categoryRepo.getSubCategoryList2(restId);
    if (response.statusCode == 200) {
      _subCategoryList = [];
      _subCategoryList
          .add(CategoryModel(id: int.parse(restId), name: 'all'.tr));
      response.body.forEach(
          (category) => _subCategoryList.add(CategoryModel.fromJson(category)));
      // getCategoryProductList(restId, '1');
    } else {
      ApiChecker.checkApi(response);
    }
  }
